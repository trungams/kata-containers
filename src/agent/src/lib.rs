...
pub async fn create_logger_task(
    rfd: RawFd,
    vsock_port: u32,
...
pub async fn real_main(init_mode: bool) -> std::result::Result<(), Box<dyn std::error::Error>> {
...
pub mod fuzz_logic {
    ...
    lazy_static! {
        pub static ref AGENT_SERVICE: std::sync::Mutex<Option<Arc<Box<dyn agent_ttrpc::AgentService + Send + Sync>>>> =
    ...
    async fn start_sandbox(
        ...
        let _server = TtrpcServer::new().register_service(aservice);
    ...
    pub fn init_agent() -> Result<()> {
        ...
        let (logger, _guard) = logging::create_logger(NAME, "fuzzing", config.log_level, writer);

        let rt = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()?;

        let (shutdown_tx, _shutdown_rx) = channel(true);
        let mut tasks = vec![];
        let init_mode = false;

        rt.block_on(start_sandbox(
            &logger,
            config,
            init_mode,
            &mut tasks,
        ...
        assert!(AGENT_SERVICE.lock().unwrap().is_some());
        Ok(())
    }
}
